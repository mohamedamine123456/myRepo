...
public class ListPartionerImpl<T> implements PartionerInterface<T> {
    ...
    @Override
    public Collection<List<T>> partionate(List<T> inputList, int partitionMaxSize) throws ListPartitionException {
        if (inputList == null) {
        ...
        Collection<List<T>> partitionList = inputList.stream()
                .collect(Collectors.groupingBy(it -> atomicInteger.getAndIncrement() / partitionMaxSize)).values();
